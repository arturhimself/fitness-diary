...
fun ICorChainDsl<ProgramContext>.stubCreateSuccess(title: String) = worker {
    this.title = title
    ...
    handle {
        ...
        val stub = ProgramStub.prepareResult {
            programRequest.id = this.id
            programRequest.title.takeIf { it.isNotBlank() }?.also { this.title = it }
            programRequest.description.takeIf { it.isNotBlank() }?.also { this.description = it }
        }
